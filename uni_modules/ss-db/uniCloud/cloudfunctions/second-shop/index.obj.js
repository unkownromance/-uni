...
const db_shop = db.collection("second-shop")
...
module.exports = {
			...
			if (userRes.code == 0) {
				...
				if (methodName == 'add') {
					...
					if (userInfoRes.data.length == 1) {
						...
						if (this.userInfo.stop_permissions.includes('add-secondshop')) {
							if (!this.userInfo.stop_permissions.includes('add-secondshop')) {
	...
	add: async function(dic) {
		...
		const res = await db_shop.add(dic)
		return {
			errCode: 0,
			errMsg: "已发布",
			data: res
	...
	edit: async function(dic) {
		...
		if (_id) {
			...
			delete(dic['zan'])
			delete(dic['collect'])
			const res = await db_shop.doc(_id).update(dic)
			return {
				errCode: 0,
				errMsg: "修改成功",
				data: res
	...
	doc: async function(_id) {
		const res = await db_shop.aggregate().match({
			_id: _id
		}).sort({
			'create_date': -1
		}).limit(1).lookup({
	...
	zan: async function(uni_id) {
		...
		if (res.data.length == 0) {
			...
			await db_shop.doc(dic.uni_id).update({
	...
	remove: async function(_id) {
		const res = await db_shop.doc(_id).get()
		console.log('删除', res)
		if (res.data.length == 1) {
			...
			const deRes = await db_shop.doc(_id).remove()
	...
	query: async function(param) {
		...
		// const res = await db_shop.where(where).orderBy('create_date', 'desc').skip(param.pageIndex).limit(10)
		// 	.get()
		const res = await db_shop.aggregate().match(where).sort({
			'create_date': -1
		}).skip(param.pageIndex).limit(10).lookup({
			from: "user-zan",
			foreignField: "uni_id",
