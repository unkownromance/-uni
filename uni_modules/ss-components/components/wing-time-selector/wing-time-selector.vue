<template>
	<view class="flex-grow">
		...
		<!-- #ifdef MP-ALIPAY -->
		<view class="flex-grow" style="flex-grow: 1;width: 100%;">
			<slot></slot>
		</view>
		<!-- #endif -->
	</view>
					...
					case 'date':
						{
							...
							this.getMonths();
							if (column == 1) {
								this.month = this.months[value].length == 3 ? this.months[value].substring(0, 2) : this.months[value].substring(
									0, 1);
							}
							this.getDays();
							this.dates[1] = this.months;
					...
					case '12':
						{
							...
							if (
								...
								(parseInt(this.beginYear) == parseInt(this.year) && parseInt(this.endYear) == parseInt(this.year) && parseInt(
									this.endMonth) == parseInt(this.month)) ||
