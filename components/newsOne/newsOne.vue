<template>
	<view>
		<!-- 左图右边文字 -->
		<view v-if="dataDic.mode == 0" class="row-start-center bg-white p-all-10  border-radius-6" @click="navToDetail">
			<image class="flex-shrink border-radius-10" :src="dataDic.cover" style="width: 240rpx;height: 180rpx;background-color: #f5f5f5;"></image>
			<view class="column-between-start flex-grow align-self-stretch m-l-10   ">
				<text class="text-maxline-two ">{{ dataDic.title }}</text>

				<view class="row-between-center align-self-stretch ">
					<text class="text-maxline-one color-999 font-24">{{ dataDic.author || '龙兴嘉苑' }}</text>
					<uni-dateformat :date="dataDic.create_date" style="color: #ccc;font-size: 24rpx;" format="yyyy/MM/dd"></uni-dateformat>
				</view>
			</view>
		</view>

		<!-- 微头条 -->
		<view v-if="dataDic.mode == 1" class="grid-one  p-lr-10 p-t-10 grid-gap-5">
			<view v-if="dataDic.author_user && !dataDic.hider" class="row-start-center">
				...
				<view class="column-between-start m-lr-10 align-self-stretch flex-grow">
					<text class="font-26 flex-grow ">{{ dataDic.author_user.nickname }}</text>
			...
			<!-- @click="navToDetail()" -->
			<view class="row-between-start" @click="navToDetail()">
				<text :class="['contentClass', detail ? 'text-autoline' : 'text-maxline-three']">{{ dataDic.content }}</text>
				<uni-dateformat
					class="flex-shrink m-l-10"
					v-if="dataDic.hider"
					:date="dataDic.create_date"
					:threshold="[0, 24 * 3600 * 1000 * 3]"
					format="yyyy/MM/dd"
					style="color: #999;font-size: 24rpx;"
				></uni-dateformat>
			</view>
			<ss-sub-meida :mediaArr="dataDic.images"></ss-sub-meida>

			<!-- 点赞与分享 收藏 -->
			<view v-if="!edit && !admin" class="grid-four ">
...
;
const newsApir = uniCloud.importObject('news-article');
