...
module.exports = {
	...
	add: async function(dic) {
		...
		const res = await db_doc.add(dic)
		return {
			errCode: 0,
			errMsg: "已发布",
			data: res
	...
	edit: async function(dic) {
		...
		if (_id) {
			...
			dic['state'] = 0
			const res = await db_doc.doc(_id).update(dic)
			return {
				errCode: 0,
				errMsg: "修改成功",
				data: res
	...
	doc: async function(_id) {
		const res = await db_doc.aggregate().match({
			_id: _id
		}).sort({
			'create_date': -1
		}).limit(1).lookup({
	...
	zan: async function(uni_id) {
		...
		if (res.data.length == 0) {
			...
			await db_doc.doc(dic.uni_id).update({
	...
	remove: async function(_id) {
		const res = await db_doc.doc(_id).get()
		console.log('删除', res)
		if (res.data.length == 1) {
			...
			const deRes = await db_doc.doc(_id).remove()
	...
	query: async function(param) {
		...
		// const res = await db_doc.where(where).orderBy('create_date', 'desc').skip(param.pageIndex).limit(10)
		// 	.get()
		const res = await db_doc.aggregate().match(where).sort({
			'sort': 1,
			'create_date': -1
		}).skip(param.pageIndex).limit(10).lookup({
			from: "uni-id-users",
	...
	// 获取资料库的分类
	getClassDetail: async function(_id) {

		const res = await db_category.doc(_id).get()
		if (res.data.length > 0) {
			return {
				errCode: 0,
				errMsg: "成功",
